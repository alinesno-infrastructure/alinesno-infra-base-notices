...
import { ref } from 'vue';
import { monitorList, closeMonitor } from '@/api/base/notice/monitor_api.js';
...
const infoForm = ref({});
    ...
    const search = () => {
      let _status = status.value === '' ? null : status.value;
      monitorList({ status: _status }).then((result) => {
        if (result.data) {
    ...
    const close = () => {
      if (infoForm.value.status == '0') {
        return false;
      }
      closeMonitor({ id: infoForm.value.id }).then((result) => {
        infoForm.value.status = '0';
        GLOBAL_FUN.successMsg();
        setTableMonitorStatus();
      });
    };

    const handleClickMonitorInfo = (row) => {
    ...
    const setTableMonitorStatus = () => {
      let length = taskList.value.length;
      for (let i = 0; i < length; i++) {
        if (taskList.value[i].id == infoForm.value.id) {
          taskList.value[i].status = infoForm.value.status;
          break;
        }
      }
    };
